...
| **G2** | 1 | "Upstream ships 85 patterns, the generator loaded 61" (`03_features.md:160`, "open item for the spec") was not taken up | **Option A.** The battery is built from the 61 and §15.4 states the gap. Whether the extractor reads the taxonomy directly is handed to `4-plan` (§14.2, §22.2). Option B, fixing the extractor first as a precondition, was not taken. **Premise corrected later** ([`10_plan.md`](../4-plan/10_plan.md) §2.1): 23 of the 24 are `quick: false` by upstream's contract; only J-1 is a generator drop |
