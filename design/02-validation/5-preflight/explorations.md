...
| Codex | codex-cli 0.154.0, ChatGPT account | Unchanged since era 01 and era 99. `gpt-5.6-luna` and `gpt-5.6-terra` available; **`gpt-5.6` is not supported on this account** (X4), so `AGENTS.md`'s Codex `large` tier cannot run |
