...
The tool facts (`--runs`, `-j`, the internal voting, the Haiku default) were then read directly from `claude plugin eval --help` rather than taken from any report. **This document used two runs**; the stage's running total is in [`README.md`](README.md).
