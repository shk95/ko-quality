...
| C1 | **Unchanged: the note recovers, it does not direct** | 2/2 runs tried `references/quick-rules.md` first |
...
### C1 — recovery is not direction

Both rewrite runs followed the same path: `SKILL.md`, then `references/quick-rules.md` (exit 1), then a directory listing, then `taxonomy-rewrite.md`. Counting era 01's run, Codex has done this in 3 of 3 attempts. Claude Code followed the note 7/7.

Review §C set the condition: "if the Codex retest shows failures, ship the file under upstream's name". Nothing failed outright. **Every run cost two extra tool calls and one failed read.** Whether that counts as failure is a decision, recorded below.
...
| R3 | 09 §7, ko-rewrite | Ship the `derived` reference under upstream's name, `references/quick-rules.md`, in **both** harnesses (check 4 requires identical skills). It is a selection, not a sentence change. This reverses era 01's P1 d1 / P6 d6 | medium: the cost is small either way |
